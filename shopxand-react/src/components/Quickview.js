  ...
  const handleSubmitReview = () => {
    ...
    fetch('http://localhost:3000/api/reviews', {
      ...
      body: JSON.stringify({
        ...
        review: { name: 'Клиент', rating: reviewRating, text: reviewText, date: new Date().toISOString() }
      })
    })
        .then(r => r.json())
    .then(data => {
      if (data.success) {
        ...
        const newReview = { name: 'Клиент', rating: reviewRating, text: reviewText, date: new Date().toISOString() };
        ...
        // Обновляем товар на главной странице
        if (typeof onUpdateProduct === 'function') {
          onUpdateProduct(product._id, newCount, newAvg);
        }
      }
    })
    .catch(() => alert('Ошибка'));
              ...
              {[product.img, ...(product.thumbs || [])].map((thumb, i) => (
                <button key={i} className={`quickview__thumb ${mainImg === thumb ? 'active' : ''}`}
                  onClick={() => {
                    ...
                    if (i > 0 && product.thumbPrices?.[i - 1]) {
                      setCurrentPrice(product.thumbPrices[i - 1]);
                    } else {
          ...
          <div className="quickview__info">
            ...
            <div className="quickview__rating">
              <div className="quickview__stars">
                {[1,2,3,4,5].map(i => (
                  <i key={i} className={`${i <= Math.round(avgRating) ? 'fas' : 'far'} fa-star`}></i>
                ))}
              </div>
              <span className="quickview__rating-num">{avgRating}</span>
              <span className="quickview__reviews">({product.reviews || 0} отзывов)</span>
            </div>
            ...
            <div className="quickview__prices">
              ...
              {product.oldPrice && (
                <span className="quickview__discount">-{Math.round((1 - product.price / product.oldPrice) * 100)}%</span>
              )}
            </div>
